...
export async function testPackages() {
  ...
  try {
    ...
    if (!firstRow || !secondRow) {
      ...
      // Don't exit process, just throw or return
      throw new Error('Insufficient rows');
    }
...
if (typeof window !== 'undefined' && window.__TEST_ENV__) {
  // Auto-run if in test env but not imported as module? 
  // Actually, we want to avoid auto-run if imported.
  // Checking if main module is tricky in ESM.
  // We'll rely on the export.
}
