export function bindCopyHandler(code, isEditMode, onCopy) {
  if (!code || code.dataset.copyBound === 'true') return;
      ...
      try {
        ...
        window.dispatchEvent(
          new CustomEvent('copy-hud', {
            detail: { x: pos.x, y: pos.y, target: code, text },
          })
        );
      } catch (_) {
...
export function registerCopyHandlers(root, isEditMode, onCopy) {
  root.querySelectorAll('code').forEach(code => bindCopyHandler(code, isEditMode, onCopy));
}
